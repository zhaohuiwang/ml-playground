""""
Keras 3 has learned from PyTorch by adopting features like dynamic graphs, eager execution, and a focus on performance. This allows Keras to combine its user-friendly API and ease of use with the flexibility and power of PyTorch.

Keras is built on top of Theano and Tensorflow. tf.Keras has more supporting languages. Simple built-in high-level API, Tensorboard for visualizing the training, production ready, good documentation.

PyTorch is based on Torch, a framework for doing fast computation that is written in C. Torch has a Lua wrapper for constructing models. PyTorch is more Pythonic, dynamic computational graphs 

If your focus is audio data alone, then PyTorch is a good choice (TorchAudio).
If you're working with text data, then TorchText is a great choice. TensorFlow Text and KerasNLP

TensorFlow has tight integration with Google Cloud, but PyTorch is integrated into TorchServe on AWS.

Dynamic vs. Static Graph Definition: In TensorFlow, the computation graphs are defined statically (TensorFlow Fold implements dynamic computation graphs in TensorFlow), whereas PyTorch has inbuilt dynamic computational graphs allowing developers to construct and modify graphs on the go.

When you run code in TensorFlow, the computation graphs (tf.Graphs contains tf.Operation and tf.Tensor) are defined statically. All communication with the outer world is performed via tf.Session object and tf.Placeholder, which are tensors that will be substituted by external data at runtime.

Distributed Training and Data parallelism: PyTorch optimizes performance by taking advantage of native support for asynchronous execution from Python (torch.nn.parallel.DistributedDataParallel or DDP module). In TensorFlow, you'll have to manually code and fine tune every operation to be run on a specific device to allow distributed training.

visualization of the training process: TensorFlow's visualization library TensorBoard is better than PyTorch's Visdom.

Production Deployment: We can directly deploy models in TensorFlow using TensorFlow serving which is a framework that uses REST Client API. PyTorch deployment framework still need to catch up.


Tensors in PyTorch and TensorFlow use C-style "row-major" ordering where incrementing the right most index corresponds to a single step in momory. If you flatten a tensor you can see what order it is laid out in memory.

# # A `-1` passed in the `shape` argument says "Whatever fits".
tf.reshape(rank_3_tensor, [-1]) 
torch.tensor.flatten() # method for PyTorch tensor
.transpose(1, 0).flatten() # to flatten a tensor in column major order
"""
