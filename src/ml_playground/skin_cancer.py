...
import numpy as np 
import pandas as pd 
# alternative file count methods
# count = 0
# for root_dir, cur_dir, files in os.walk(benign_dir.as_posix()):
#    count += len(files)
# print('file count:', count)
# len([item for item in benign_dir.rglob("*") if item.is_file()])

# perform os.path.join(path-to-dir file-name) to get the full path for each image
...
# bar plot for distributions of two type of cancers
plt.figure(figsize=(15, 8))  

ax = sns.countplot(x='label', data=df, palette='hls')
ax.set_ylim(0, df['label'].value_counts().max() * 1.1)  
...
# pie plot for distributions of two type of cancers
...
# pick up 5 images for each type and plot them into two rows
...
# the sample sized are not equal for two tupe of cancers. here to balance them 
...
df_resampled.to_parquet(Path(path)/'df_sampled.parquet')
